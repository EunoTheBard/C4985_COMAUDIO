...
int waveIn(TCHAR *fileName, LPHWAVEOUT lphWave,LPWAVEFORMATEX lpwfe,LPWAVEHDR lpwhdr)
{
	HANDLE fhWave;
	int iFileSize;
	LPBYTE lpBuf;
	...
	if(fhWave==INVALID_HANDLE_VALUE){
		CloseHandle(fhWave);
		MessageBox(NULL, TEXT("cannot open file"),fileName,MB_OK);
		return 1;
	}
	...
	formType[3]=lpBuf[11];
	formType[4]='\0';
	if(_tcscmp(formType,TEXT("WAVE")) != 0){
		HeapFree(GetProcessHeap(),0,lpBuf);
		MessageBox(NULL, TEXT("Specify wav file"),fileName,MB_OK);
		return 2;
	}
	...
	if(lpwfe->wFormatTag != WAVE_FORMAT_PCM){
		HeapFree(GetProcessHeap(),0,lpBuf);
		MessageBox(NULL,TEXT("Spefify PCM type of wav file "),fileName,MB_OK);
		return 3;
	}
